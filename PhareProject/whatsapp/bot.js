...
client.on("message", async (msg) => {
  ...
  if (attente[from]) {
    ...
    if (!isNaN(choix) && choix >= 1 && choix <= docs.length) {
      ...
      try {
        ...
        await msg.reply(media, { sendMediaAsDocument: true, caption: `📄 ${doc.nom}` });
      } catch (e) {
  ...
  if (resultat.docs && resultat.docs.length > 0) {
    ...
    attente[from] = docs;
    if (docs.length === 1) {
      await msg.reply(`📄 J'ai trouvé : *${docs[0].nom}*\n_${docs[0].description}_\n\nEnvoi en cours...`);
      try {
        const media = await MessageMedia.fromUrl(docs[0].url, { unsafeMime: true });
        await msg.reply(media, { sendMediaAsDocument: true, caption: `📄 ${docs[0].nom}` });
        delete attente[from];
      } catch (e) {
    ...
    await msg.reply(resultat.message || "Je n'ai rien trouvé. Essaie avec d'autres mots-clés.");
  }
});
